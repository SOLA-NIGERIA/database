...
CREATE TABLE IF NOT EXISTS party.source_describes_party
(
...
CREATE TABLE IF NOT EXISTS party.source_describes_party_historic
(
...
CREATE TABLE IF NOT EXISTS system.consolidation_config
(
...
BEGIN
  ...
  for table_rec in select table_name from information_schema.tables where table_schema = schema_name loop
    ...
    sql_part = (select 'CREATE TABLE IF NOT EXISTS ' || schema_name || '.' || table_rec.table_name || '(' 
...
BEGIN
  ...
  execute 'CREATE TABLE IF NOT EXISTS ' || consolidation_schema || '.config (
  ...
  for table_rec in select * from system.consolidation_config order by order_of_execution loop
    ...
    sql_to_run = 'CREATE TABLE IF NOT EXISTS ' || consolidation_schema || '.' || table_rec.table_name 
