...
ALTER TABLE administrative.ba_unit_as_party ADD COLUMN rowidentifier character varying(40) not null default uuid_generate_v1();
...
CREATE OR REPLACE  FUNCTION get_text_from_schema_only(schema_name character varying) RETURNS text
    LANGUAGE plpgsql
    AS $$
DECLARE
  table_rec record;
...
INSERT INTO consolidation_config (id, schema_name, table_name, condition_description, condition_sql, remove_before_insert, order_of_execution, nr_rows_at_once, log_in_extracted_rows) VALUES  ('administrative.source_describes_dispute_historic', 'administrative', 'source_describes_dispute_historic', 'Every record that references a record in the main table in consolidation schema.', 'rowidentifier in (select rowidentifier from consolidation.source_describes_dispute)', false, 91, 10000, false);
INSERT INTO consolidation_config (id, schema_name, table_name, condition_description, condition_sql, remove_before_insert, order_of_execution, nr_rows_at_once, log_in_extracted_rows) VALUES  ('cadastre.sr_work_unit', 'cadastre', 'sr_work_unit', 'Every record', '', true, 92, 10000, false);
