...
INSERT INTO cadastre.spatial_unit (id, dimension_code, label, abreviated_code, surface_relation_code, geom, level_id, change_user) 
	SELECT uuid_generate_v1(), '2D', adm2, (case
 
when interim_data.lga.adm2='Albasu'
then 'ABS'
when interim_data.lga.adm2='Bagwai'
then 'BGW'
when interim_data.lga.adm2='Bebeji'
then 'BBJ'
when interim_data.lga.adm2='Bichi'
then 'BCH'
when interim_data.lga.adm2='Bunkure'
then 'BNK'
when interim_data.lga.adm2='Dala'
then 'DAL'
when interim_data.lga.adm2='Nassaraw'
then 'NSR'
when interim_data.lga.adm2='Dambatta'
then 'DBT'
when interim_data.lga.adm2='Gezawa'
then 'GZW'
when interim_data.lga.adm2='DawakinK'
then 'DKK'
when interim_data.lga.adm2='DawakinT'
then 'DKT'
when interim_data.lga.adm2='Gabasawa'
then 'GBS'
when interim_data.lga.adm2='Gaya'
then 'GYA'
when interim_data.lga.adm2='Gwarzo'
then 'GRZ'
when interim_data.lga.adm2='Kabo'
then 'KAB'
when interim_data.lga.adm2='Karaye'
then 'KRY'
when interim_data.lga.adm2='Kumbotso'
then 'KBT'
when interim_data.lga.adm2='Kura'
then 'KUR'
when interim_data.lga.adm2='Minjibir'
then 'MJB'
when interim_data.lga.adm2='Rano'
then 'RAN'
when interim_data.lga.adm2='RiminGad'
then 'RMG'
when interim_data.lga.adm2='Shanono'
then 'SNN'
when interim_data.lga.adm2='Sumaila'
then 'SML'
when interim_data.lga.adm2='Takai'
then 'TAK'
when interim_data.lga.adm2='Tsanyawa'
then 'TYW'
when interim_data.lga.adm2='Ungogo'
then 'UGG'
when interim_data.lga.adm2='Wudil'
then 'WDL'
when interim_data.lga.adm2='Kabo'
then 'KAB'
when interim_data.lga.adm2='Fagge'
then 'FGE'
...
INSERT INTO cadastre.spatial_unit (id, dimension_code, label,abreviated_code, surface_relation_code, geom, level_id, change_user) 
	SELECT uuid_generate_v1(), '2D', name,'UGG', 'onSurf--TO POPULATE THE SOLA DATABASE WITH SHAPEFILE DATA FOR KANO
--INTO SOLA KANO DATABASE
--modified Paola Rizzo 17 May 2013
...
when interim_data.lga.adm2='Dala'
then 'DAL'
when interim_data.lga.adm2='Nassarawa'
then 'NSR'
when interim_data.lga.adm2='Dambatta'
then 'DBT'
when interim_data.lga.adm2='Gezawa'
...
INSERT INTO cadastre.spatial_unit_group( id, hierarchy_level, label, name, geom, change_user) 
	SELECT adm1||'/'||adm2, 2, adm2, 'KN'||'/'||(case
...
ace', the_geom, (SELECT id FROM cadastre.level WHERE name='Ward') As l_id, 'test' AS ch_user 
	FROM interim_data.wards WHERE (ST_GeometryN(the_geom, 1) IS NOT NULL);
...
INSERT INTO cadastre.spatial_unit_group( id, hierarchy_level, label, name, geom, change_user, seq_nr)
select lga_group.name || '/' || w.name, 3, w.name, lga_group.name || '/' || (case w.name when w.name='Ungogo 1'
then 'UGG 1'
else w.name
end), w.the_geom, 'test', 0
from cadastre.spatial_unit_group as lga_group,  interim_data.wards as w 
