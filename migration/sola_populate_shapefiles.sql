...
--	FROM interim_data.lga WHERE (ST_GeometryN(the_geom, 1) IS NOT NULL);
--INSERT INTO cadastre.spatial_unit_group( name,id, hierarchy_level, label,  change_user) SELECT distinct('NI'),('NI'), 0, 'NIGERIA', 'test';
--UPDATE cadastre.spatial_unit_group set name = 'NI', label = 'NIGERIA' where hierarchy_level=0;
--------------- State

--INSERT INTO cadastre.spatial_unit_group( name,id, hierarchy_level, label,  change_user) SELECT distinct(adm1),adm1, 1, 
--(adm1), 'test'
--	FROM interim_data.lga WHERE (ST_GeometryN(the_geom, 1) IS NOT NULL);
--INSERT INTO cadastre.spatial_unit_group( name,id, hierarchy_level, label,  change_user) SELECT distinct('KD'),('KD'), 1, 'KADUNA', 'test';
--UPDATE cadastre.spatial_unit_group set name = 'KD', label = 'KADUNA' where hierarchy_level=1;
--------------- LGA
--INSERT INTO cadastre.spatial_unit_group( id, hierarchy_level, label, name, geom, change_user) 
--	SELECT adm1||'/'||adm2, 2, adm2, adm1||'/'||adm2
--	, the_geom, 'test'
--	FROM interim_data.lga WHERE (ST_GeometryN(the_geom, 1) IS NOT NULL);
--UPDATE cadastre.spatial_unit_group set name = 'KD/1', label = 'BIRNIN GWARI' where hierarchy_level=2 and label='1';
...
--NSERT INTO cadastre.spatial_unit_group( id, hierarchy_level, label, name, geom, change_user, seq_nr)
--SELECT lga_group.name || '/' || w.name, 3, w.name, 
--CASE 	WHEN lga_group.name = 'Kano/Ungogo' THEN 'KN/UNG/' || trim(left(w.name, position(' ' in w.name))) || substring(w.name from position(' ' in w.name) + 1 for (char_length(w.name) - position(' ' in w.name)))
