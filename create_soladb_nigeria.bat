...
for /f "eol=: delims=" %%F in (
  'dir %changesetPath%\*.sql /b /a-d /one   2^>nul'
) do %psql_path% --host=%host% --port=5432 --username=%username% --dbname=%dbname% --file=%changesetPath%\%%F  >> build.log 2>&1


for /f "eol=: delims=" %%F in (
  'dir ..\database-%label%\%changesetPath%\*.sql /b /a-d /one   2^>nul'
) do %psql_path% --host=%host% --port=5432 --username=%username% --dbname=%dbname% --file=..\database-%label%\%changesetPath%\%%F  >> build.log 2>&1
